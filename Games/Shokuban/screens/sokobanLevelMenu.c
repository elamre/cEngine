...
static void create(Screen * screen)
{
	...
	sokobanLevelMenu->levelSelected = levelProgress;
	levelSelect(sokobanLevelMenu,levelProgress);
}
...
static void buttonDraw(GuiButton * guiButton)
{
	...
	if(levelButton->level <= levelProgress){
		graphical_drawRectangle(guiButton->x + 1,guiButton->y + 1,guiButton->x+guiButton->width-3,guiButton->y+guiButton->height-3,1,RED);
	} else {
		graphical_drawRectangle(guiButton->x + 1,guiButton->y + 1,guiButton->x+guiButton->width-3,guiButton->y+guiButton->height-3,1,GRAY);
	}
	...
	graphical_drawRectangle(guiButton->x + 1,guiButton->y + 1,guiButton->x+guiButton->width - 1,guiButton->y+guiButton->height - 1,0,BLACK);
}
